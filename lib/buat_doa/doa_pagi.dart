...
import 'package:project_app/database/doa_CRUD_pagi_database.dart';

class DoaPagi extends StatefulWidget {
  const DoaPagi({super.key});

  @override
  State<DoaPagi> createState() => _DoaPagiState();
}

class _DoaPagiState extends State<DoaPagi> {
  ...
  Future<void> _loadDoaList() async {
    final tambahDoa = await DoaCrudDatabase.getTambahDoaList();
    setState(() {
      _doaList = tambahDoa;
    });
  }
  ...
  Future<void> _deleteDoa(int id) async {
    await DoaCrudDatabase.deleteDoa(id);
    _loadDoaList();
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (doaData == null) {
                  // Tambah Doa
                  await DoaCrudDatabase.insertDoa(nama, arab, arti);
                } else {
                  // Edit Doa
                  final id = doaData['id'];
                  await DoaCrudDatabase.updateDoa(id, nama, arab, arti);
                }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _doaList.isEmpty
          ? const Center(
        child: Text("Doa Pagi Belum di isi",style: TextStyle(fontSize: 18, fontStyle: FontStyle.italic)
        ),
      )
          :ListView(
        children: [
