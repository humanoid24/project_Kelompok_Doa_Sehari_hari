...
import 'package:project_app/database/doa_CRUD_siang_database.dart';

class DoaSiang extends StatefulWidget {
  const DoaSiang({super.key});

  @override
  State<DoaSiang> createState() => _DoaSiangState();
}

class _DoaSiangState extends State<DoaSiang> {
  ...
  Future<void> _loadDoaList() async {
    final tambahDoa = await DoaCrudDatabaseSiang.getTambahDoaListSiang();
    setState(() {
      _doaList = tambahDoa;
    });
  }
  ...
  Future<void> _deleteDoa(int id) async {
    await DoaCrudDatabaseSiang.deleteDoaSiang(id);
    _loadDoaList();
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (doaData == null) {
                  // Tambah Doa
                  await DoaCrudDatabaseSiang.insertDoaSiang(nama, arab, arti);
                } else {
                  // Edit Doa
                  final id = doaData['id'];
                  await DoaCrudDatabaseSiang.updateDoaSiang(id, nama, arab, arti);
                }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _doaList.isEmpty
          ? const Center(
            child: Text("Doa Siang Belum di isi",style: TextStyle(fontSize: 18, fontStyle: FontStyle.italic)
            ),
          )
    : ListView(
        children: [
